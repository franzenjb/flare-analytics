    ...
    <div className="bg-white rounded p-5 border border-arc-gray-100">
      ...
      {deltas && deltas.length > 0 && (
        <div className="flex flex-wrap gap-3 mt-2">
          {deltas.map((d, i) => (
            <div key={i} className={`flex items-center gap-1 text-[10px] font-[family-name:var(--font-data)] ${d.good ? 'text-arc-success' : 'text-arc-red'}`}>
              {d.value >= 0 ? <TrendingUp size={10} /> : <TrendingDown size={10} />}
